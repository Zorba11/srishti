...
interface Props {
  ...
  movieId: string;
  movieSysPrompt: string;
}

export const IdeaPromptForm = observer(
  ({ ideaStore, baseUserPrompt, movieId, movieSysPrompt }: Props) => {
    const [userPrompt, setUserPrompt] = useState(baseUserPrompt);
    const [systemPrompt, setSystemPrompt] = useState(
      movieSysPrompt ?? ideaStore.systemPrompt
    );
        ...
        >
          ...
          <div className="flex gap-4 pt-4">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              disabled={ideaStore.isGenerating}
              onClick={() =>
                ideaStore.generateIdeas({ userPrompt, systemPrompt }, movieId)
              }
